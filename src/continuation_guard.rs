...
/// No file in this crate has yet adopted the "hand-aligned `\n`-joined banner" idiom
/// (network_info.rs et al. in dig-node). Once a line has committed to that idiom -- it
/// contains a literal `\n` escape anywhere -- every space run on it is column
/// alignment, not a torn sentence, so the whole line would be exempt.
const CLI_COLUMN_FILES: &[&str] = &[];

/// A lost continuation always leaves the source's own indentation as a mid-sentence
/// space run (14-22 spaces in the sibling crates this guard was ported from); ordinary
/// column-alignment padding never exceeds 8. Ten leaves margin on both sides:
/// comfortably above every legitimate pad, comfortably below the smallest real defect.
/// Shared across every crate that carries this guard -- do not invent a second
/// discriminator (dig_ecosystem#3130).
...
/// This crate has dozens of source files; a scan that reads too few (a wrong
...
const MIN_FILES_SCANNED: usize = 50;
